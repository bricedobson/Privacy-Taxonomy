...
2 & Identify input taxonomies & 45 accepted, +9 supplemental \\
...
7 & Final reconciliation & gaps discussed with the W3C \\
\hline
\end{tabular}
\end{table}
...
validator informed by a selection of evaluation criteria drawn from Staab and Studer's \emph{Handbook on Ontologies}~\cite{staab2009handbook}, where the validator was built using RDFLib~\cite{rdflib}. The validator applied test cases that checked for likely missing entities, format gaps, missing relationships, missing attributes, typing gaps, mismatched domains, and mismatched output ranges.
...
Running the suite began an iterative process (run the validator, resolve apparent issues, run it again) that converged at 12 iterations. At this point, those that remained were decisions reserved for the implementer (Section~\ref{sec:risk}) and represented stylistic choices of approach and risk tolerance, rather than the construction of the data model. The model's footprint changed approximately 12\% between the pre- and post-evaluation states, with results mostly ``building out'' cross-domain relationships and necessary reified entities in the ``middle'' of the data model. I assert this degree of change establishes the initial evaluation was substantive rather than confirmatory.
...
27002~\cite{iso27002} are modeled as a class hierarchy or an attribute facet has no correct answer in the abstract, but carries material consequences for how controls are queried, inherited, and reported once thousands of instances exist.
...
\begin{enumerate}
    ...
    \item There may be merit in using the model as an evaluation instrument against commercial tools with the aim of pinpointing exactly what maturity level their implicit data structures imply, extend to, or max out at.
\end{enumerate}
...
Lastly, I believe there is a lesson here on approach. Ontologies, taxonomies, and reference schemas exist for thousands of data applications. Further, standards of any type imply entities and relationships where one may not exist. Each of these creates a mechanism to take a data-driven approach to problem solving and process maturity issues for that space. There is real value in an approach that ideates on what the data would look like if the problem no longer existed and working backward from that, where getting better is a function of getting more precise, which is tactically applied through forced data normalization.
\clearpage
...
\begin{itemize}
    \item Dr. Brenden Kuerbis and Mr. Emad AbuKhousa for their continued review of this content throughout the course it was a product of.
...
\item This document was compiled using \href{https://github.com/MemoJimenez/Rho-class}{Rho-class}, a \LaTeX{} template developed by Guillermo Jimenez.
\end{itemize}
...
\printbibliography[heading=none,filter=taxonomystandards,env=nolabelbib]
\subsubsection*{Governmental}
\printbibliography[heading=none,filter=taxonomyregulatory,env=nolabelbib]
\subsubsection*{Commercial}
\printbibliography[heading=none,filter=taxonomycommercialtools,env=nolabelbib]
\subsubsection*{Academic Work}
\printbibliography[heading=none,filter=taxonomyacademicwork,env=nolabelbib]
\clearpage
\phantomsection
...
\clearpage
\subsection*{Appendix C: Summary Analysis Statistics}
\label{app:e}
...
\toggletrue{trackcited}
\clearpage
\subsection*{Appendix D: Risk-Discretionary Implementation Decisions}
\label{app:decisions}
...
\caption{The risk-discretionary implementation decisions.}
\label{tab:decisions}\\
